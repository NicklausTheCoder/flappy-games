...
class CheckersMultiplayer {
  ...
  async getLobby(lobbyId: string): Promise<CheckersLobby | null> {
    ...
    try {
      ...
      return res.success && res.lobby ? res.lobby as CheckersLobby : null;
    } catch (err) {
  ...
  async setPlayerReady(lobbyId: string, uid: string, isReady: boolean): Promise<void> {
    try {
      ...
      console.log(`✅ [Checkers] Player ${uid} ready=${isReady} written to lobby ${lobbyId}`);
    } catch (err) {
  ...
  async markLobbyReady(_lobbyId: string): Promise<void> {}
