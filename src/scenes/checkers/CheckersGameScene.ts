...
export class CheckersGameScene extends Phaser.Scene {
    ...
    private createUI() {
        ...
        // Restart button - small at top right
        const restartBtn = this.add.text(320, 10, '🔄', {
            fontSize: '24px',
            color: '#ffffff',
        ...
        // Back button - small at top left
        const backBtn = this.add.text(40, 10, '←', {
            fontSize: '24px',
            color: '#ffffff',
            backgroundColor: '#4a4a4a',
            padding: { x: 10, y: 5 }
            ...
            .setInteractive({ useHandCursor: true })
            .on('pointerdown', () => this.scene.start('CheckersStartScene'));
    }
    ...
    private gameOver(message: string) {
        ...
        const gameOverText = this.add.text(180, 300, message, {
            fontSize: '32px',
            color: '#ffff00',
            fontStyle: 'bold',
            stroke: '#000000',
        ...
        // Add restart prompt
        this.add.text(180, 350, 'Tap to play again', {
            fontSize: '18px',
            color: '#ffffff'
        }).setOrigin(0.5);
